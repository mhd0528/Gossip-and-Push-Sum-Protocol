defmodule Proj2bonus.Server2 do
    use GenServer
    ...
    def handle_cast({:updateNeighbour, _failNodesList, randomNodeList}, psState) do
        
        currentActorIndex=Enum.at(psState,0)
        if(Enum.member?(randomNodeList,currentActorIndex)==true) do
            newState=[Enum.at(psState,0),Enum.at(psState,1),Enum.at(psState,2),Enum.at(psState,3),0,[]] 
            {:noreply, newState}
        else
            {:noreply,psState}
        end                 
    ...
    def handle_cast({:pushSum, swValues},psState) do
        ...
        if(rounds<3) do
            ...
            newPsState=[Enum.at(psState,0),newS/2,newW/2,newRounds,Enum.at(psState,4),Enum.at(psState,5)]
            checkFlag=Enum.at(newPsState,4)
            if(checkFlag===0) do
                Process.sleep(5000)
                Process.exit(self(),:normal)
                neighbourList=sendValuesToNeighbour(Enum.at(newPsState,5),newS/2,newW/2)
                {:noreply,newPsState}
            else
                neighbourList=sendValuesToNeighbour(Enum.at(newPsState,5),newS/2,newW/2)
                {:noreply,newPsState}
